...
public class FlightService {
    ...
    public OffsetDateTime updateFlightTracking(Long flightId, TrackingDto dto) {
        ...
        if (dto.getAction().equals(FlightAction.START.getName())) {
            ...
            flight.get().getPlane().addFlightHours(
                (double) Duration.between(flight.get().getActualStartTime(), currentTime).getSeconds()
                    / 3600.0);
            ...
            OffsetDateTime currentFlightDateTime = DateFusioner
                .fusionStartDate(flight.get().getStartDate(), flight.get().getScheduledFlight().getStartTime(), null);
    ...
    private OffsetDateTime calculateNextWorkingDay(OffsetDateTime currentFlightDateTime) {
        OffsetDateTime nextWorkingDay = currentFlightDateTime
        ...
        if (currentFlightDateTime.isBefore(currentFlightDateTime.withHour(6).withMinute(0).withSecond(0).withNano(0))) {
            nextWorkingDay = currentFlightDateTime
    ...
    private List<Flight> getPlaneFlightsForCurrentAndNextDay(Plane plane, OffsetDateTime currentFlightDateTime, OffsetDateTime nextWorkingDay) {
    ...
    private List<Flight> filterPendingFlightsForCurrentWorkday(List<Flight> flights, OffsetDateTime currentFlightDateTime, OffsetDateTime nextWorkingDay) {
    ...
    private boolean cascadeDelay(OffsetDateTime currentFlightDateTime, OffsetDateTime nextWorkingDay, Flight alreadyDelayedFlight, double delay) {
        ...
        if (leftoverDelayForPlane > 15) {
            ...
            for (int i = 0; i < currentDayFlights.size(); i++){
                ...
                if ((leftoverDelayForPlane - bufferMinutes) > 0) {
                    ...
                    OffsetDateTime delayedFlightDateTime = DateFusioner
                        .fusionStartDate(currentDayFlights.get(i).getStartDate(), currentDayFlights.get(i).getScheduledFlight().getStartTime(), null);
                    if (!cascadeDelay(delayedFlightDateTime, nextWorkingDay, currentDayFlights.get(i), leftoverDelayForPlane)) {
                        return false;
                    }
        ...
        if (leftoverDelayForPilot > 60) {
            ...
            if (pilot != null) {
                List<FlightCrewMember> memberEntries = flightCrewMemberRepository.findByEmployeeUserId(pilot.getEmployeeUserId());
                ...
                currentDayPilotFlights = filterPendingFlightsForCurrentWorkday(pilotTempFlights, currentFlightDateTime, nextWorkingDay);
                ...
                for (int i = 0; i < currentDayPilotFlights.size(); i++) {
                    ...
                    if(i-1 >=0){
                        i1= DateFusioner.fusionArrivalDate(currentDayPilotFlights.get(i-1).getStartDate(),
                            ...
                            currentDayPilotFlights.get(i-1).getScheduledFlight().getDurationInHours(),
                            null ) .toInstant();
                    }
                    else{
                        i1= DateFusioner.fusionArrivalDate(alreadyDelayedFlight.getStartDate(),
                    ...
                    Instant i2 =DateFusioner.fusionStartDate(currentDayPilotFlights.get(i).getStartDate(),
                        currentDayPilotFlights.get(i).getScheduledFlight().getStartTime(),
                        null )
                        .toInstant();
                    ...
                    if ((leftoverDelayForPilot - bufferMinutes) > 0) {
                        ...
                        OffsetDateTime delayedFlightDateTime = DateFusioner
                            .fusionStartDate(currentDayPilotFlights.get(i).getStartDate(), currentDayPilotFlights.get(i).getScheduledFlight().getStartTime(), null);
                        if (!cascadeDelay(delayedFlightDateTime, nextWorkingDay, currentDayPilotFlights.get(i), leftoverDelayForPilot)) {
                            return false;
                        }

                    }
                }
                flightRepository.saveAll(currentDayPilotFlights);
            }
        }
