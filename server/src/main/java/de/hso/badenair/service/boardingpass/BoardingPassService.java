...
public class BoardingPassService {
	...
	/**
	 * Retrieves the required data from the database and generates a boarding pass
	 * pdf using this data.
	 * 
	...
	public byte[] getBoardingPass(Long id) {
		...
		List<Luggage> luggage = luggageRepository.findAllByTravelerId(traveler.get().getId());
	 ...
	 * 
	 * @param traveler the traveler data (this object should include references to
	 *                 the flight data as well)
	 * @param luggage  the list of luggage which the traveler has booked
	 * @return the generated boarding pass pdf document
	 * @throws IOException if writing to the pdf document fails (thrown by the
	 *                     pdfbox library)
	 */
	private byte[] generatePdf(Traveler traveler, List<Luggage> luggage) throws IOException {
		// Duration
		Double durationInHours = traveler.getBooking().getFlight().getScheduledFlight().getDurationInHours();
		int minutes = (int) ((durationInHours - durationInHours.intValue()) * 60.0);
		String duration = durationInHours.intValue() + " Stunden, " + minutes + " Minuten";

		// Start date
		OffsetDateTime startDate = DateFusioner.fusionStartDate(traveler.getBooking().getFlight().getStartDate(),
				traveler.getBooking().getFlight().getScheduledFlight().getStartTime(),
				traveler.getBooking().getFlight().getScheduledFlight().getStartingAirport().getTimezone());
		String startDateString = startDate.format(DateTimeFormatter.ofPattern("dd. MMMM yyyy, HH:mm")) + " Uhr ("
				+ startDate.getOffset().getDisplayName(TextStyle.SHORT, Locale.GERMAN) + ")";

		// Arrival date
		OffsetDateTime arrivalDate = DateFusioner.fusionArrivalDate(traveler.getBooking().getFlight().getStartDate(),
				traveler.getBooking().getFlight().getScheduledFlight().getStartTime(), durationInHours,
				traveler.getBooking().getFlight().getScheduledFlight().getStartingAirport().getTimezone());
		String arrivalDateString = arrivalDate.format(DateTimeFormatter.ofPattern("dd. MMMM yyyy, HH:mm")) + " Uhr ("
				+ arrivalDate.getOffset().getDisplayName(TextStyle.SHORT, Locale.GERMAN) + ")";
		...
		qrCodeStringBuilder.append("\n--------\n\nFlugnummer: ");
		qrCodeStringBuilder.append(traveler.getBooking().getFlight().getScheduledFlight().getId().toString());
		qrCodeStringBuilder.append("\nVon: ");
		qrCodeStringBuilder
				.append(traveler.getBooking().getFlight().getScheduledFlight().getStartingAirport().getName());
		qrCodeStringBuilder.append("\nNach: ");
		qrCodeStringBuilder
				.append(traveler.getBooking().getFlight().getScheduledFlight().getDestinationAirport().getName());
		qrCodeStringBuilder.append("\nFlugdauer: ");
		...
		try {
			...
			contents.drawImage(qrCode, 612.0f - border - qrCodeSize, 792.0f - border - yLength, qrCodeSize, qrCodeSize);
		...
		FileInputStream logoInputStream = new FileInputStream(ResourceUtils.getFile("classpath:logo2.png"));
		PDImageXObject logo = PDImageXObject.createFromByteArray(document, IOUtils.toByteArray(logoInputStream),
				"logo2.png");
		logoInputStream.close();
		float logoOffset = 5.0f;
		float logoSizeX = 585.0f * 0.14f;
		float logoSizeY = 375.0f * 0.14f;
		contents.drawImage(logo, border + logoOffset, 792.0f - border - logoSizeY - logoOffset, logoSizeX, logoSizeY);

		// Demo ad
		FileInputStream adInputStream = new FileInputStream(ResourceUtils.getFile("classpath:ad_scaled.png"));
		PDImageXObject ad = PDImageXObject.createFromByteArray(document, IOUtils.toByteArray(adInputStream),
				"ad_scaled.png");
		adInputStream.close();
		...
		contents.lineTo(612.0f - border, 792.0f - border - logoSizeY - logoOffset);

		// Vertical line in header
		contents.moveTo(612.0f - border - logoSizeY - logoOffset, 792.0f - border - logoSizeY - logoOffset);
		contents.lineTo(612.0f - border - logoSizeY - logoOffset, 792.0f - border);

		// Second horizontal line
		float secondLineOffset = 117.0f;
		contents.moveTo(border, 792.0f - border - logoSizeY - logoOffset - secondLineOffset);
		contents.lineTo(612.0f - border, 792.0f - border - logoSizeY - logoOffset - secondLineOffset);
		...
		// Passenger
		contents.beginText();
		contents.newLineAtOffset(border + logoSizeX + 14.0f, 792.0f - border - 22.0f);
		...
		contents.showText(traveler.getFirstName() + " " + traveler.getLastName());
		contents.endText();

		// Seat number
		String[] seatLetters = { "A", "B", "C", "D", "E", "F" };
		contents.beginText();
		contents.newLineAtOffset(612.0f - border - logoSizeY + 1.0f, 792.0f - border - 22.0f);
		contents.setNonStrokingColor(Color.DARK_GRAY);
		contents.setFont(fontBold, 11);
		...
		contents.setNonStrokingColor(Color.BLACK);
		contents.setFont(fontNormal, 14);
		contents.showText((traveler.getSeatRow() + 1) + "" + seatLetters[traveler.getSeatColumn()]);
		contents.endText();
		...
		// Start/destination
		contents.beginText();
		contents.newLineAtOffset(border + 14.0f, 792.0f - border - logoSizeY - 24.0f);
		contents.setNonStrokingColor(Color.DARK_GRAY);
		...
		contents.showText(traveler.getBooking().getFlight().getScheduledFlight().getId().toString());
		contents.newLine();
		contents.setNonStrokingColor(Color.DARK_GRAY);
		contents.setFont(fontBold, 10);
		...
		contents.setNonStrokingColor(Color.BLACK);
		contents.setFont(fontNormal, 13);
		contents.showText(traveler.getBooking().getFlight().getScheduledFlight().getStartingAirport().getName());
		contents.newLine();
		contents.setNonStrokingColor(Color.DARK_GRAY);
		contents.setFont(fontBold, 10);
		...
		contents.setNonStrokingColor(Color.BLACK);
		contents.setFont(fontNormal, 13);
		contents.showText(traveler.getBooking().getFlight().getScheduledFlight().getDestinationAirport().getName());
		contents.endText();

		// Time stuff
		contents.beginText();
		contents.newLineAtOffset(612.0f / 2.0f + 14.0f, 792.0f - border - logoSizeY - 24.0f);
		contents.setNonStrokingColor(Color.DARK_GRAY);
		contents.setFont(fontBold, 10);
		...
		// Luggage
		contents.beginText();
		contents.newLineAtOffset(border + 14.0f, 792.0f - border - logoSizeY - logoOffset - secondLineOffset - 34.0f);
		contents.setNonStrokingColor(Color.DARK_GRAY);
		contents.setFont(fontBold, 10);
		...
		contents.endText();
		contents.beginText();
		contents.newLineAtOffset((612.0f - qrCodeSize) / 2.0f + 14.0f,
				792.0f - border - logoSizeY - logoOffset - secondLineOffset - 34.0f);
		contents.setNonStrokingColor(Color.DARK_GRAY);
		contents.setFont(fontBold, 10);
	 ...
	 */
	private byte[] generateQRCode(String input) throws WriterException, IOException {
	...
	/**
	 * Returns the string to display in the boarding pass for a given piece of
	 * luggage.
	 * 
	 * @param list  the list of booked luggage (can be smaller than the requested
	 *              index)
